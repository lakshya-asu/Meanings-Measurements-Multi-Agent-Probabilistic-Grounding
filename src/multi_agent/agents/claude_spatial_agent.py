...
class ClaudeSpatialAgent:
    ...
    def process(self, blackboard: Blackboard, anchor_obj: Dict[str, Any]) -> Dict[str, Any]:
        if not blackboard.current_image_path or not os.path.exists(blackboard.current_image_path):
            blackboard.append_event("Spatial(Claude)", "Error", "No image for spatial kernel", "FAIL")
        ...
        Schema:
        {json.dumps(SpatialOutput.model_json_schema(), indent=2)}
        """
        ...
        try:
            ...
            blackboard.append_event("Spatial(Claude)", "Error", str(e), "FAIL")
