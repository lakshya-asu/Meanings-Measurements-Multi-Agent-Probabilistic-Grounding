...
class OpenAIOrchestratorAgent:
    ...
    def process(self, blackboard: Blackboard) -> Dict[str, Any]:
        ...
        try:
            ...
            blackboard.append_event("Orchestrator(OpenAI)", "ParseQuery", parsed, "PASS")
            return parsed
        except Exception as e:
            error_msg = f"Failed to orchestrate query (OpenAI): {e}"
            blackboard.append_event("Orchestrator(OpenAI)", "Error", error_msg, "FAIL")
